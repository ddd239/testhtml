...
import {InteractionState, Annotation} from "./history_cluster_types.mojom-webui.js";
...
<div id="backgroundsDisabled" hidden$="[[!customBackgroundDisabledByPolicy_]]">
  ...
  <div id="backgroundsDisabledTitle">관리자가 맞춤 백그라운드를 사용 중지했습니다.</div>
</div>
...
<div id="options">
  <div id="optionCustomLinks" class$="option [[getCustomLinksSelected_(customLinksEnabled_, hide_)]]">
    <cr-button id="optionCustomLinksButton" class="option-image" tabindex="0" aria-pressed$="[[getCustomLinksAriaPressed_(customLinksEnabled_,
            hide_)]]" title="내 바로가기" on-click="onCustomLinksClick_" noink>
    ...
    <div class="option-title">내 바로가기</div>
    <div class="option-description">내가 선별한 바로가기입니다.</div>
  </div>
  <div id="optionMostVisited" class$="option [[getMostVisitedSelected_(customLinksEnabled_, hide_)]]">
    <cr-button id="optionMostVisitedButton" class="option-image" tabindex="0" aria-pressed$="[[getMostVisitedAriaPressed_(customLinksEnabled_,
            hide_)]]" title="자주 방문한 사이트" on-click="onMostVisitedClick_" on-keydown="onMostVistedKey" noink>
      <div class="option-icon"></div>
      <div class="option-mini">
        <ntp-mini-page single-colored-logo></ntp-mini-page>
      </div>
    ...
    <div class="option-title">자주 방문한 사이트</div>
    <div class="option-description">자주 방문하는 웹사이트를 기반으로 추천된 바로가기입니다.</div>
  </div>
</div>
<div id="hide" class$="[[getHideClass_(hide_)]]">
  <div id="hideIcon" class="cr-icon icon-visibility-off"></div>
  <div id="hideTitleContainer">
    <div id="hideTitle">바로가기 숨기기</div>
    <div id="hideDescription" class="option-description">
      이 페이지에 바로가기를 표시하지 않습니다.
    </div>
  </div>
  <cr-toggle id="hideToggle" title="바로가기 숨기기" checked="[[hide_]]" on-change="onHideChange_"></cr-toggle>
</div>
<!--_html_template_end_-->`
}
// Copyright 2020 The Chromium Authors
...
<div id="container" hidden>
  <div id="show">
    <cr-radio-group selected="[[radioSelection_(show_)]]" disabled="[[showManagedByPolicy_]]" on-selected-changed="onShowRadioSelectionChanged_">
      <cr-radio-button id="hideButton" name="hide" label="모든 카드 숨기기">
      </cr-radio-button>
      <cr-radio-button id="customizeButton" name="customize" label="카드 맞춤설정">
      </cr-radio-button>
    </cr-radio-group>
      ...
      <div class="toggle-section">
        ...
        <template is="dom-if" if="[[showCartOptionToggle_(item.id, item.checked)]]">
          <div id="cartOption" class="toggle-option-row">
            ...
            <cr-toggle checked="{{cartOptionToggle_.enabled}}" title="장바구니"></cr-toggle>
          </div>
        </template>
        <template is="dom-if" if="[[showDiscountOptionToggle_(item.id, item.checked,
          ...
          <div id="discountOption" class="toggle-option-row">
            ...
            <cr-toggle checked="{{discountOptionToggle_.enabled}}" title="할인받기"></cr-toggle>
          </div>
        </template>
      </div>
    </template>
  ...
  <div slot="title">
    ...
    <div id="title">
      ...
      <div id="titleNavigation" hidden="[[!showTitleNavigation_]]">
        <cr-icon-button id="backButton" class="icon-arrow-back" on-click="onBackClick_" title="뒤로">
        </cr-icon-button>
...
<div id="promoAndDismissContainer" hidden>
  ...
  <template is="dom-if" if="[[shownMiddleSlotPromoId_]]">
    <cr-icon-button id="dismissPromoButton" class="icon-clear" aria-label="닫기" title="닫기" on-click="onDismissPromoButtonClick_">
    </cr-icon-button>
  </template>
</div>
<cr-toast id="dismissPromoButtonToast" duration="10000">
  ...
  <cr-button id="undoDismissPromoButton" aria-label="실행취소하려면 Ctrl+Z을(를) 누르세요." on-click="onUndoDismissPromoButtonClick_">
    실행취소
  </cr-button>
</cr-toast>
<!--_html_template_end_-->`
...
<div id="titleContainer">
  ...
  <template is="dom-if" if="[[showInfoButton]]">
    <cr-icon-button id="infoButton" title="카드 정보" on-click="onInfoButtonClick_">
    </cr-icon-button>
  </template>
...
<div id="consentCardContainer">
  ...
  <template is="dom-if" if="[[showCloseButton_]]" restamp>
    <cr-icon-button id="close" class="icon-clear" aria-label="닫기" on-click="onCloseClick_"></cr-icon-button>
  </template>
</div>
          ...
          <template is="dom-if" if="[[!discountConsentHasTwoSteps_]]" restamp>
            <div id="consentCard">
              ...
              <span id="consentContent">
                Chrome이 내 장바구니의 상품에 적용되는 맞춤 할인을 찾도록 허용하시겠습니까?
              </span>
...
let HistoryClustersModuleElement$1 = class HistoryClustersModuleElement extends (I18nMixin(PolymerElement)) {
    ...
    getInfo_(discounts) {
        ...
        return this.i18nAdvanced(hasDiscount ? "modulesHistoryWithDiscountInfo" : "modulesJourneysInfo")
    }
}
;
...
async function createElement$1() {
    ...
    if (loadTimeData.getBoolean("historyClustersModuleDiscountsEnabled")) {
        const {discounts: discounts} = await HistoryClustersProxyImpl$1.getInstance().handler.getDiscountsForCluster(clusters[0]);
      ...
      <div id="optInContent">
        <div>
          ...
          <span id="optInTitleDesc">
            로그인할 때마다 Google 포토에서 소중한 추억들을 만나보세요.
          </span>
        </div>
        <div>
...
class DriveModuleElement extends (I18nMixin(PolymerElement)) {
    ...
    getMenuItemGroups_() {
        return [[{
            ...
            text: this.i18nRecursive("", "modulesDismissButtonText", "modulesDriveFilesLower")
        }, {
            action: "disable",
            icon: "modules:block",
            text: this.i18nRecursive("", "modulesDisableButtonTextV2", "modulesDriveSentenceV2")
        }, {
            action: "info",
            icon: "modules:info",
...
class CartTileModuleElementV2 extends (I18nMixin(PolymerElement)) {
    ...
    getImagesToShow_() {
        ...
        if (!this.showRelatedSearches && this.format === "wide") {
            ...
            if (images.length >= 4) {
                if (this.imagesEnabled || this.showRelatedSearches && this.format === "wide") {
    ...
    shouldShowExtraImagesCard_() {
        return this.showRelatedSearches ? this.cart.productImageUrls.length > 2 : true
    }
    getExtraImagesCount_() {
        const images = this.cart.productImageUrls;
        if (!this.showRelatedSearches && this.format === "wide") {
...
const template$2 = html`<iron-iconset-svg name="modules" size="24">
  <svg>
    <defs>
...
class ModuleHeaderElementV2 extends (I18nMixin(PolymerElement)) {
    ...
    onButtonClick_(e) {
        const {action: action} = e.model.item;
        assert(action);
        e.stopPropagation();
        this.$.actionMenu.close();
...
class HistoryClustersHeaderElementV2 extends ElementBase {
    static get is() {
    ...
    onClick_(e) {
        e.stopPropagation();
        this.dispatchEvent(new CustomEvent("show-all-button-click",{
            bubbles: true,
            composed: true
        }))
    }
    onSuggestClick_(e) {
        e.stopPropagation();
        this.dispatchEvent(new CustomEvent("suggest-click",{
            bubbles: true,
            composed: true
        }))
    }
...
class HistoryClustersProxyImpl {
    ...
    static getInstance() {
        ...
        return instance$3 = new HistoryClustersProxyImpl(handler)
    }
    static setInstance(obj) {
        instance$3 = obj
    }
...
class HistoryClustersModuleElement extends (I18nMixin(PolymerElement)) {
    ...
    onDoneButtonClick_() {
        HistoryClustersProxyImpl.getInstance().handler.updateClusterVisitsInteractionState(this.cluster.visits, InteractionState.kDone);
        this.dispatchEvent(new CustomEvent("dismiss-module-instance",{
            bubbles: true,
            composed: true,
            detail: {
...
async function createElement(cluster) {
    ...
    element.discounts = [];
    if (loadTimeData.getBoolean("historyClustersModuleDiscountsEnabled")) {
        const {discounts: discounts} = await HistoryClustersProxyImpl.getInstance().handler.getDiscountsForCluster(cluster);
        ...
        const hasDiscount = element.discounts.slice(0, CLUSTER_MIN_REQUIRED_URL_VISITS).some((discount=>discount.length > 0));
        chrome.metricsPrivate.recordBoolean(`NewTabPage.HistoryClusters.HasDiscount`, hasDiscount)
    } else {
...
class MostVisitedBrowserProxy {
    ...
    static getInstance() {
        ...
        instance$2 = new MostVisitedBrowserProxy(handler,callbackRouter);
        return instance$2
    }
    static setInstance(obj) {
        instance$2 = obj
...
class MostVisitedWindowProxy {
    ...
    static getInstance() {
        return instance$1 || (instance$1 = new MostVisitedWindowProxy)
    }
    static setInstance(obj) {
        instance$1 = obj
    }
...
class PageImageServiceBrowserProxy {
    ...
    static getInstance() {
        return instance || (instance = new PageImageServiceBrowserProxy(PageImageServiceHandler.getRemote()))
    }
    static setInstance(obj) {
        instance = obj
    }
      ...
      <template is="dom-if" if="{{isNormalOrError_}}">
        ...
        <div id="urlUploadContainer">
          ...
          <div id="inputContainer">
            <input id="inputBox" autocomplete="false" autocorrect="false" placeholder="이미지 링크 붙여넣기" text="text" value="{{uploadUrl_::input}}" on-keydown="onUrlKeyDown_">
...
<cr-dialog id="dialog" show-close-button on-cancel="onDismissClick_" show-on-attach close-text="닫기">
  <div slot="title">장바구니</div>
  <div slot="body">
    <div id="content-container" tabindex="0">
      <div id="content-title">할인을 받으시겠습니까?</div>
      <div id="content">Google이 내 장바구니의 상품에 적용되는 할인을 찾도록 허용하시겠습니까?</div>
    </div>
  ...
  <template is="dom-if" if="[[modulesFreShown]]">
    <div id="firstRunExperience">
      ...
      <div class="first-run-experience-body">
        카드를 선택하고 나에게 중요한 주제에 관한 최신 소식을 받아보세요.
      </div>
      <div class="first-run-experience-body">
        <a is="action-link" id="customizeChromeLink" tabIndex=0 on-click="onCustomizeModuleFre_">Chrome 맞춤설정</a>에서 모든 카드 옵션을 확인하세요.
      </div>
...
<cr-toast id="removeModuleToast" duration="10000">
  ...
  <template is="dom-if" if="[[removedModuleData_.undo]]">
    <cr-button id="undoRemoveModuleButton" aria-label="실행취소하려면 Ctrl+Z을(를) 누르세요." on-click="onUndoRemoveModuleButtonClick_">
      실행취소
    </cr-button>
  </template>
</cr-toast>
<cr-toast id="removeModuleFreToast" duration="10000">
  <div id="removeModuleFreToastMessage">
    카드가 이 페이지에 더 이상 표시되지 않습니다.
  </div>
  <cr-button id="undoRemoveModuleFreButton" aria-label="실행취소하려면 Ctrl+Z을(를) 누르세요." on-click="onUndoRemoveModuleFreButtonClick_">
    실행취소
  </cr-button>
</cr-toast>
<!--_html_template_end_-->`
...
function getTemplate() {
    return html`<!--_html_template_start_--><style include="cr-hidden-style">#container{display:flex;flex-flow:row wrap;gap:var(--container-gap);justify-content:center;margin-top:8px;max-width:var(--container-max-width,inherit)}#undoToastMessage{flex-grow:1}</style>
<div id="container">
</div>
<template>
...
<cr-toast id="undoToast" duration="10000">
  ...
  <template is="dom-if" if="[[undoData_.undo]]">
    <cr-button id="undoButton" aria-label="실행취소하려면 Ctrl+Z을(를) 누르세요." on-click="onUndoButtonClick_">
      실행취소
    </cr-button>
  </template>
</cr-toast>
